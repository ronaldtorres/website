...
Most iOS projects don't have to deal with this because they use [Match](https://fastlane.tools),
...
1. Create an **unlocked** keychain where signing certificates will be imported.
   If we use the system one, the OS will prompt the user to confirm the access,
...
Since we are using Ruby and [Rake](https://github.com/ruby/rake) for automation in the project,
below I include some code snippets that I extracted from the project.
...
As we can notice in the code snippet below,
