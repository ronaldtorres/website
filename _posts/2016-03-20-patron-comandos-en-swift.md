...
Como nuestro comando debía ser añadido a una cola de ejecución que determinara la concurrencia y el hilo de ejecución de estos decidimos definir la clase `Command` a partir de `NSOperation` pero definiéndolo como una clase genérica respecto al tipo retornado y el error que pueda retornar la ejecución:
